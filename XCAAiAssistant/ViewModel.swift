...
class ViewModel: NSObject, AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    ...
    #if !os(macOS)
    var recordingSeesion = AVAudioSession.sharedInstance()
    #endif
    ...
    override init() {
        ...
        do {
            #if os(iOS)
            try recordingSeesion.setCategory(.playAndRecord, options: .defaultToSpeaker)
            #else
            try recordingSeesion.setCategory(.playAndRecord, mode: .default)
            #endif
            try recordingSeesion.setActive(true)
